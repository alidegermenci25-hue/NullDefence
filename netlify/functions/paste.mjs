...
async function blobSet(key, value) {
  const res = await fetch(`${BLOBS_BASE}/${key}`, {
    method: "PUT",
    headers: {
...
export const handler = async (event) => {
  ...
  if (httpMethod === "POST") {
    try {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Failed to create paste", detail: err.message }),
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
      };
    }
  }
  ...
  if (httpMethod === "GET") {
    ...
    if (!id) {
      return { statusCode: 400, body: "Missing paste ID" };
    }

    try {
      const content = await blobGet(id);
